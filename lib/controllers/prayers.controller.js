...
class PrayersController {
    constructor() {
        this.prayerManagerValidator = async (request, response, next) => {
            this._validationController.validationMiddlewareByObject(this._prayerManager, validators.PrayerMangerValidator.createValidator());
        };
        this.prayerSettingsRequestValidator = async (request, response, next) => {
        };
        this.prayerViewMobileRequestValidator = async (request, response, next) => {
            next(this._validationController
                .validationMiddlewareByRequest(1 /* body */, validators.ConfigValidator.createValidator()));
        };
